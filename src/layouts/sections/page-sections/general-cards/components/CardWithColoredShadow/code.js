          ...
          {cards.map(({ color, title, desc }) => (
            <Grid item xs={12} md={6} lg={4} key={title}>
              <MKBox
                ...
                shadow="md"
                p={3}
                sx={{ boxShadow: ({ boxShadows: { colored } }) => colored[color] }}
              >
                <MKTypography variant="h5" color={color} mb={1}>{title}</MKTypography>
                <MKTypography variant="body2" color="text">{desc}</MKTypography>
              </MKBox>
            </Grid>
          ))}
        </Grid>

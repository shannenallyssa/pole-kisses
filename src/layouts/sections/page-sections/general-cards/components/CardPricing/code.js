        ...
        <Grid container spacing={3} justifyContent="center">
          {plans.map(({ name, price, color, features }) => (
            <Grid item xs={12} md={6} lg={4} key={name}>
              <MKBox bgColor="white" borderRadius="xl" shadow="md" p={4} textAlign="center">
                <MKTypography variant="h5" mb={1}>{name}</MKTypography>
                <MKTypography variant="h2" color={color} mb={3}>
                  {price}
                  <MKTypography component="span" variant="body2" color="text">/mo</MKTypography>
                </MKTypography>
                {features.map((feature) => (
                  <MKBox key={feature} display="flex" alignItems="center" mb={1}>
                    <Icon color="success" sx={{ mr: 1 }}>check</Icon>
                    <MKTypography variant="body2" color="text">{feature}</MKTypography>
                  </MKBox>
                ))}
